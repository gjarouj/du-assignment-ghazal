  ...
  switch (type) {
    ...
    case "secondary":
      return `
            ...
            ${
              size && size === "small"
                ? "min-w-auto h-fit text-[0.8rem] py-1 px-3"
                : "min-w-[7rem] h-full p-2"
            }
            bg-white
            border
            rounded-[0.3125rem]
...
const Button: React.FC<ButtonProps> = ({ type, size, label }) => (
  <button className={`font-medium ${generateClassList(type, size)}`}>
    {label}
  </button>
);
